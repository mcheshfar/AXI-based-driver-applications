...
always_comb
	if(!in_key_empty && !out_key_full)
		...
		out_key_din[(8)`ith] <= in_key_dout[(8)`ith] ^ in_key_dout[(8-4)`ith];
		out_key_din[(8+1)`ith] <= in_key_dout[(8+1)`ith] ^ in_key_dout[(8-3)`ith];
		out_key_din[(8+2)`ith] <= in_key_dout[(8+2)`ith] ^ in_key_dout[(8-2)`ith];
		out_key_din[(8+3)`ith] <= in_key_dout[(8+3)`ith] ^ in_key_dout[(8-1)`ith];
