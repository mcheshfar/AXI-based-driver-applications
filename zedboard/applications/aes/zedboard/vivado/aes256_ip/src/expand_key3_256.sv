...
always_comb
	if(!in_key_empty && !out_key_full && !round_key_full)
		...
		out_key_din[(12)`ith] <= in_key_dout[(12)`ith] ^ in_key_dout[(12-4)`ith];
		out_key_din[(12+1)`ith] <= in_key_dout[(12+1)`ith] ^ in_key_dout[(12-3)`ith];
		out_key_din[(12+2)`ith] <= in_key_dout[(12+2)`ith] ^ in_key_dout[(12-2)`ith];
		out_key_din[(12+3)`ith] <= in_key_dout[(12+3)`ith] ^ in_key_dout[(12-1)`ith];
		...
		round_key_din <= out_key_din[127 : 0];
		end
