...
module expand_key7_256(
	...
	output reg [127 : 0] round_key_din,
	output reg round_key_write,
	input round_key_full,
...
always_comb
	if(!in_key_empty && !out_key_full && !round_key_full)
		begin
		in_key_read <= 1;
		out_key_write <= 1;		
		round_key_write	<= 1;
		...
		out_key_din[4`ith] <= in_key_dout[4`ith];
		out_key_din[5`ith] <= in_key_dout[5`ith];
		out_key_din[6`ith] <= in_key_dout[6`ith];
		out_key_din[7`ith] <= in_key_dout[7`ith];
		...
		out_key_din[(28)`ith] <= in_key_dout[(28)`ith] ^ in_key_dout[(28-4)`ith];
		out_key_din[(28+1)`ith] <= in_key_dout[(28+1)`ith] ^ in_key_dout[(28-3)`ith];
		out_key_din[(28+2)`ith] <= in_key_dout[(28+2)`ith] ^ in_key_dout[(28-2)`ith];
		out_key_din[(28+3)`ith] <= in_key_dout[(28+3)`ith] ^ in_key_dout[(28-1)`ith];	

		round_key_din <= out_key_din[255 : 128];
		end
