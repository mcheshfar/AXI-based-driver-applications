...
module read_key_256(
	input [31 : 0] in_token_dout,
	output reg in_token_read,
	input in_token_empty,
	
	output reg [255 : 0] out_token_din,
...
always_comb
	if (counterTokens < N_TOKEN && !in_token_empty)
		...
		en_partial_token = 1;
		next_partial_token = partial_token;
		next_partial_token[counterTokens *32 +: 32] = in_token_dout;
		end
	
	else if(counterTokens == N_TOKEN && !out_token_full)
		begin
        in_token_read = 0;
		out_token_din = partial_token;
		out_token_write = 1;
		next_CounterTokens = 0;
		en_partial_token = 1;
		next_partial_token = 0;		
		end
