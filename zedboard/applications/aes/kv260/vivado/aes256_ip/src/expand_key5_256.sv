...
always_comb
	if(!in_key_empty && !out_key_full)
		...
		out_key_din[(20)`ith] <= in_key_dout[(20)`ith] ^ in_key_dout[(20-4)`ith];
		out_key_din[(20+1)`ith] <= in_key_dout[(20+1)`ith] ^ in_key_dout[(20-3)`ith];
		out_key_din[(20+2)`ith] <= in_key_dout[(20+2)`ith] ^ in_key_dout[(20-2)`ith];
		out_key_din[(20+3)`ith] <= in_key_dout[(20+3)`ith] ^ in_key_dout[(20-1)`ith];	
