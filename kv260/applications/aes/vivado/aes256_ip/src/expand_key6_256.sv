...
always_comb
	if(!in_key_empty && !out_key_full)
		...
		out_key_din[(24)`ith] <= in_key_dout[(24)`ith] ^ in_key_dout[(24-4)`ith];
		out_key_din[(24+1)`ith] <= in_key_dout[(24+1)`ith] ^ in_key_dout[(24-3)`ith];
		out_key_din[(24+2)`ith] <= in_key_dout[(24+2)`ith] ^ in_key_dout[(24-2)`ith];
		out_key_din[(24+3)`ith] <= in_key_dout[(24+3)`ith] ^ in_key_dout[(24-1)`ith];
