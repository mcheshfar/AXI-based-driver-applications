...
always_comb
	if(!in_key_empty && !in_rc_empty && !out_key_full)
		...
		out_key_din[0`ith] <= in_key_dout[0`ith] ^ sub_byte(in_key_dout[29`ith]) ^ in_rc_dout;
		out_key_din[1`ith] <= in_key_dout[1`ith] ^ sub_byte(in_key_dout[30`ith]);
		out_key_din[2`ith] <= in_key_dout[2`ith] ^ sub_byte(in_key_dout[31`ith]);
		out_key_din[3`ith] <= in_key_dout[3`ith] ^ sub_byte(in_key_dout[28`ith]);
